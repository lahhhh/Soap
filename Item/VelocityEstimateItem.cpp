...
void VelocityEstimateItem::__set_menu() {
...
void VelocityEstimateItem::s_show_embedding_velocity_by_stream() {

	this->show_embedding_velocity(1);
}

void VelocityEstimateItem::s_show_embedding_velocity_by_grid() {
...
void VelocityEstimateItem::show_embedding_velocity(int mode) {
	...
	ShowEmbeddingVelocityWorker* worker = new ShowEmbeddingVelocityWorker(this->data(), *embedding, settings, mode);

	if (mode == 0) {
		G_LINK_WORKER_THREAD(ShowEmbeddingVelocityWorker, x_grid_graph_ready, VelocityEstimateItem, s_receive_velocity_grid);
	}
	else {
		G_LINK_WORKER_THREAD(ShowEmbeddingVelocityWorker, x_stream_graph_ready, VelocityEstimateItem, s_receive_velocity_stream);
	}
};

std::pair<QCustomPlot*, QCPAxisRect*> VelocityEstimateItem::draw_feature_plot(
...
void VelocityEstimateItem::s_receive_velocity_stream(STREAM_PLOT_ELEMENTS res)
{
...
void VelocityEstimateItem::s_receive_velocity_grid(VELO_GRID_PLOT_ELEMENTS res)
{
	auto [draw_area, axis_rect] = this->draw_feature_plot(res.embedding, res.embedding_names, res.graph_settings);
