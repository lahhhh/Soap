...
void EnrichWorker::enrich_pathway() {
    ...
    EnrichmentDatabase db;
    
    if (this->database_ == "GO") {
        CustomMatrix enrichment = db.enrich_go(this->gene_names_, this->ontology_, this->species_, this->p_adjust_method_, this->p_threshold_);
        if (enrichment.is_empty()) {
            G_TASK_WARN("Enrichment in GO database failed.");

        }
        else {
            emit x_enrichment_ready(enrichment, "GO " + this->enrich_type_ + " " + this->ontology_);
        }
    }
    else {
        CustomMatrix enrichment = db.enrich_kegg(this->gene_names_, this->species_, this->p_adjust_method_, this->p_threshold_);
        if (enrichment.is_empty()) {
            G_TASK_WARN("Enrichment in KEGG database failed.");

        }
        else {
            emit x_enrichment_ready(enrichment, "KEGG " + this->enrich_type_);
        }
    }
};
