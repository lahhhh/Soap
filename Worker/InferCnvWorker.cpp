...
void InferCnvWorker::finalize() {
	...
	for (auto&& [type, ind] : this->ref_grouped_cell_indices_) {
		...
		auto subcluster = custom::reordered(this->subclusters_, ind);

		auto clusters = custom::unique(subcluster);

		for (auto&& cluster : clusters) {

			auto ind2 = custom::match(subcluster, cluster);

			cell_ind << custom::reordered(ind, ind2);
		}
...
bool InferCnvWorker::prepare_0() {
	...
	QVector<int> starts = custom::cast<int>(gene_location->get_qstring("Column 3"));
	QVector<int> stops = custom::cast<int>(gene_location->get_qstring("Column 4"));
	...
	for (const auto& chr : unique_chromosome) {
		auto chr_filter = custom::equal(chromosome_list, chr);
		QStringList chr_gene_list = custom::sliced(gene_list, chr_filter);
		auto gene_filter = custom::in(chr_gene_list, this->counts_.rownames_);
		int n_gene = gene_filter.count();
		if (n_gene > 20) {

			this->chrs_ << QStringList(n_gene, chr);

			gene_index << custom::valid_index_of(chr_gene_list, this->counts_.rownames_);

			this->starts_ << custom::sliced(custom::sliced(starts, chr_filter), gene_filter);

			this->stops_ << custom::sliced(custom::sliced(stops, chr_filter), gene_filter);
		}
	}
	...
	Eigen::ArrayX<bool> filter = custom::col_sum(this->counts_.mat_) > this->min_counts_per_cell_;
	...
	this->ref_group_names_ = custom::intersect(this->ref_group_names_, custom::unique(this->metadata_));
	...
	this->obs_group_names_ = custom::set_difference(custom::unique(this->metadata_), this->ref_group_names_);
	
	for (auto&& level : this->ref_group_names_) {
		this->ref_grouped_cell_indices_[level] = custom::match(this->metadata_, level);
	}

	for (auto&& level : this->obs_group_names_) {
		this->obs_grouped_cell_indices_[level] = custom::match(this->metadata_, level);
	}
...
bool InferCnvWorker::remove_insufficiently_expressed_genes_1() {
	...
	Eigen::ArrayX<bool> filter1 = custom::row_mean(this->counts_.mat_) >= this->cut_off_;

	Eigen::ArrayX<bool> filter2 = custom::row_count(this->counts_.mat_, 1) > this->min_cells_per_gene_;
	...
	this->chrs_ = custom::sliced(this->chrs_, filter);
	this->starts_ = custom::sliced(this->starts_, filter);
	this->stops_ = custom::sliced(this->stops_, filter);
	return true;
};
...
bool InferCnvWorker::normalization_by_sequencing_depth_2() {
	...
	custom::normalize_in_place(this->expr_);
	...
	//int n_genes = custom::sum(custom::sapply(this->fake_chr_info_, [](auto&& t) {return std::get<2>(t); }));

	//auto genes_means_use_idx = custom::sample_integer(0, this->expr_.rows(), n_genes);
	...
	//		if (cnv != 1.0) {
	//			hspike_gene_means(custom::match(this->fake_chrs_, name)) *= cnv;
	//		}
	//	}
	...
	//		sim_counts_matrix = custom::cbind(sim_normal_matrix, sim_spiked_cnv_matrix);
	//	}
	//	else {
	//		sim_counts_matrix = custom::cbind(sim_counts_matrix, sim_normal_matrix, sim_spiked_cnv_matrix);
	//	}

	//	this->fake_ref_grouped_cell_indices_[type] = custom::seq_n(cell_counter, n_cells);

	//	cell_counter += n_cells;

	//	this->fake_obs_grouped_cell_indices_[type] = custom::seq_n(cell_counter, n_cells);
	...
	//custom::normalize_in_place(this->fake_expr_); // to do
...
bool InferCnvWorker::log_transformation_3() {
	...
	if (this->scale_data_) {
		...
		custom::scale_in_place(this->expr_);

		if (this->hmm_) {
			custom::scale_in_place(this->fake_expr_);
		}
...
bool InferCnvWorker::smooth_6() {
	...
	auto chrs = custom::unique(this->chrs_);

	for (auto&& chr : chrs) {

		auto ind = custom::match(this->chrs_, chr);
	...
	auto fake_chrs = custom::unique(this->fake_chrs_);

	for (auto&& chr : fake_chrs) {

		auto ind = custom::match(this->fake_chrs_, chr);
...
bool InferCnvWorker::center_7() {
	...
	auto meds = custom::sapply(custom::seq_n(0, this->expr_.cols()), [this](auto&& i) {return custom::median(this->expr_.col(i)); });

	this->expr_.array().rowwise() -= custom::cast<Eigen::ArrayX>(meds).transpose();

	if (!this->hmm_) {
		return true;
	}

	meds = custom::sapply(custom::seq_n(0, this->fake_expr_.cols()), [this](auto&& i) {return custom::median(this->fake_expr_.col(i)); });

	this->fake_expr_.array().rowwise() -= custom::cast<Eigen::ArrayX>(meds).transpose();
...
bool InferCnvWorker::compute_tumor_subcluster_11() {
	...
	if (this->z_score_filter_ > 0.0) {
		...
		auto ind = custom::unroll(custom::sapply(this->ref_grouped_cell_indices_, [](auto&& it) {return it.second; }));

		Eigen::MatrixXd ref_expr = expr(Eigen::all, ind);
		Eigen::MatrixXd z_score = (ref_expr.array() - ref_expr.mean()) / custom::sd(ref_expr);
		Eigen::ArrayX<bool> filter = z_score.array().abs().rowwise().mean() > this->z_score_filter_;

		if (filter.count() > 0) {
			expr = custom::row_sliced(expr, custom::flip(filter));
			chrs = custom::sliced(chrs, custom::flip(filter));
		}
	}
	...
	for (auto&& [type, ind] : this->ref_grouped_cell_indices_) {
		...
		auto pca = pca_infercnv(group_expr, custom::min(2000, group_expr.rows() / 2), 10);
		if (pca.size() == 0) {
		...
		custom::assign(this->subclusters_, partition, ind);
	}
...
bool InferCnvWorker::hmm_13() {
	...
	auto fake_obs_ind = custom::unroll(custom::sapply(this->fake_obs_grouped_cell_indices_, [](auto&& it) {return it.second; }));
	...
	auto cnv_levels = custom::unique(spike_cnv);
	...
	for (auto&& level : cnv_levels) {
		Eigen::MatrixXd level_expr = this->fake_expr_(custom::match(spike_cnv, level), Eigen::all);

		double mean = level_expr.mean();

		double sd = custom::sd(level_expr);
		...
		sds = custom::sliced(sds, filter);

		Eigen::ArrayXd n_cells = Eigen::ArrayXd::LinSpaced(100, 2, 101);

		n_cells = custom::sliced(n_cells, filter);
	...
	auto tumor_subclusters = custom::unique(this->subclusters_);
	...
	for (auto&& chr : chrs) {
		...
		auto chr_ind = custom::match(this->chrs_, chr);
		...
		for (auto&& cluster : tumor_subclusters) {
			...
			auto subcluster_ind = custom::match(this->subclusters_, cluster);
			...
			double sd = custom::median(custom::sapply(cluster_cnv_mean_sd, [](auto&& it) {return it.second.second; }));
			...
			if (custom::any(custom::sapply(nu.row(n - 1), [](auto&& d) {return std::isnan(d) || std::isinf(d); }), true)) {
				G_TASK_WARN("Underflow");
				return false;
			}

			y(n - 1) = custom::argmax(nu.row(n - 1));

			for (int i = n - 2; i > -1; --i) {
				y(i) = custom::argmax(logpi.col(y(i + 1)) + nu.row(i).transpose());
			}
...
bool InferCnvWorker::generate_reports_14() {
	...
	/*auto subclusters = custom::unique(this->subclusters_);
	auto chrs = custom::unique(this->chrs_);
	...
	for (auto&& cluster : subclusters) {
		Eigen::MatrixXi cell_group_mtx = this->hmm_data_(Eigen::all, custom::match(this->subclusters_, cluster));
		auto state_consensus = custom::sapply(custom::seq_n(0, n_gene),
			[this](int i) {return custom::find_most_frequent_element(this->hmm_data_.row(i)); });

		for (auto&& chr : chrs) {

			auto gene_idx = custom::match(this->chrs_, chr);
			auto chr_state = custom::reordered(state_consensus, gene_idx);
...
//
//		mean << custom::cast<QVector>(custom::row_mean(expr_sub.mat_));
//		p0 << custom::cast<QVector>(1.0 - custom::row_count<double, true, false>(expr_sub.mat_, 0).cast<double>() / ind.size());
//		var << custom::cast<QVector>(custom::row_var(expr_sub.mat_));
//	}
//
...
//	}
//
//	auto filter = custom::greater_than(mean, 0.0);
//	Eigen::ArrayXd x = log(custom::cast<Eigen::ArrayX>(custom::sliced(mean, filter)));
//	Eigen::ArrayXd y = custom::cast<Eigen::ArrayX>(custom::sliced(p0, filter));
//
...
//
//	Eigen::ArrayXd logm = log(custom::cast<Eigen::ArrayX>(mean) + 1.0);
//	Eigen::ArrayXd logv = log(custom::cast<Eigen::ArrayX>(var) + 1.0);
//
